...
// Storage keys (shared with Encounters):
//   BESTIARY_KEY: 'tp_bestiary_v1'       // full authoring data (admin-only)
//   PUBLISH_DATA: 'tp_enemies_data_v1'   // minimal schema for Encounters
//   PUBLISH_PING: 'tp_enemies_public_v1' // ping for storage event updates
...
(() => {
  ...
  /** @typedef {ReturnType<typeof defaultEnemy>} Enemy */
  ...
  function edit(id){
    ...
    byId('str').value = ab.str ?? 10;
    byId('dex').value = ab.dex ?? 10;
    byId('con').value = ab.con ?? 10;
    byId('int').value = ab.int ?? 10;
    byId('wis').value = ab.wis ?? 10;
    byId('cha').value = ab.cha ?? 10;
    // misc
    byId('skills').value = e.skills || '';
    byId('senses').value = e.senses || '';
    byId('languages').value = e.languages || '';
    byId('traits').value = e.traits || '';
    byId('actions').value = e.actions || '';
    byId('reactions').value = e.reactions || '';
    byId('legendary').value = e.legendary || '';
    byId('source').value = e.source || '';
  ...
  function serializeForm(){
    ...
    return {
      ...
      abilities: {
        str: int(byId('str').value, 10),
        dex: int(byId('dex').value, 10),
        con: int(byId('con').value, 10),
        int: int(byId('int').value, 10),
        wis: int(byId('wis').value, 10),
        cha: int(byId('cha').value, 10),
      },
  ...
  function upsertEnemy(data){
    ...
    if(idx === -1){ state.list.unshift(data); }
    else { state.list[idx] = data; }
  ...
  // ---------- Validation (Admin-side; blocks publish with precise reasons) ----------
  ...
  function validateArray(arr){
    ...
    arr.forEach((e, i) => {
      ...
      if (!e.type || typeof e.type !== 'string') warnings.push(`${where} '${e.name||"?"}': missing 'type' (recommended)`);

      // Duplicates
      if (e.id && seenIds.has(e.id)) errors.push(`${where} '${e.name||"?"}': duplicate id '${e.id}'`);
      seenIds.add(e.id);

      // Abilities optional but checked if present
      ...
      // OK → accept
      const hasHard = errors.some(msg => msg.startsWith(where));
  ...
  function publish(){
    // Build minimal schema for Encounters and compute XP from CR table
    ...
    if (warnings.length){
      console.group('[Enemy Admin] Publish warnings');
      warnings.forEach(w => console.warn(w));
      console.groupEnd();
    }
    alert(`Published ${valid.length} enemies to Encounters. Key: ${PUBLISH_DATA}`);
  }
  ...
  function quickGenerate(){
    ...
    const templates = [
      ...
      { name:'Bandit', size:'Medium', type:'Humanoid', ac:12, hp:11, cr:'1/8',
        ...
        traits:'Pack Tactics (variant).',
        actions:'Scimitar. +3 to hit; Light Crossbow. +3 to hit.',
