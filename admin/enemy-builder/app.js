...
// after localStorage.setItem(... PUBLISH_DATA ...) and PUBLISH_PING:
try {
  const bc = new BroadcastChannel('tp_enemies');
  bc.postMessage({ type: 'publish', updatedAt: Date.now(), count: valid.length });
  bc.close?.();
} catch {}

  // resolve admin path from script tag data-admin-path, else default
    ...
    arr.forEach((e,i)=>{
      ...
      if(e.abilities && typeof e.abilities==='object'){
        for(const k of ['str','dex','con','int','wis','cha']){
          ...
          if(v!=null && !isIntIn(v,1,30)) errors.push(`${where} '${e.name||"?"}': abilities.${k} must be 1..30 (got ${v})`);
        }
      }
  ...
  if(bc){
    bc.onmessage = (ev)=> {
      if(ev && ev.data && ev.data.type==='publish') load();
    };
  }
